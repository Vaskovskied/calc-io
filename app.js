  ...
  button.addEventListener("click", (e) => {
    ...
    if (b === "" && sign === "") {
      if (a.toString().length === 18) {
        if (
          ACTIONS.includes(btnText) === false &&
          ACTIONSwithoutB.includes(btnText) === false &&
          btnText !== "«"
        ) {
          return;
        }
    ...
    if (btnText === "√") {
      const setError = () => {
        clearAll();
        $output.innerText = "error";
        $outputInfo.innerText = "you're smart enough to check it";
        setTimeout(() => {
          clearAll();
        }, 3000);
      };
...
function decreaseFontSize(btnText) {
  ...
  let outputInfoFontSize = parseInt(
    window.getComputedStyle($outputInfo).fontSize
  );
  ...
  if (
    $outputInfo.innerText.length >= 12 &&
    outputInfoFontSize >= 14 &&
    btnText !== "C" &&
    (btnText !== "«") & (btnText !== ".")
  ) {
    if ((24 / $outputInfo.innerText.length) * 14 >= 10) {
      $outputInfo.style.fontSize = `${
        (24 / $outputInfo.innerText.length) * 14
      }px`;
    } else {
      $outputInfo.style.fontSize = `${
        (24 / $outputInfo.innerText.length) * 14 + 2
      }px`;
    }
  }
}
...
function overflowError() {
  ...
  if ($output.innerText.length > 18 || $output.innerText === "Infinity") {
    ...
    $outputInfo.innerText =
      "You maliciously or curiously try to break program. The clear all function will be executed in 3 seconds";
...
function createHistoryElement(info, result, emptiness) {
  ...
  $historyDisplay.addEventListener("click", (e) => {
    ...
    $outputInfo.innerText =
      $historyDisplay.querySelector(".history-output").innerText;
    $output.innerText = a;
